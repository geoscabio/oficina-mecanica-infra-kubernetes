...
- Não introduza, durante destroy, `apply` normal, apply direcionado, recriação de EKS/ECR ou fluxo que transforme recuperação em provisionamento.
- Trate outputs e recursos EKS/ECR/SSM já ausentes como recuperação idempotente somente quando a resposta AWS confirmar ausência; `AccessDenied`, token expirado, credencial inválida e demais falhas reais devem bloquear.
- Confirme a limpeza do status Kubernetes no SSM sem mascarar falhas de consulta como ausência do parâmetro.
